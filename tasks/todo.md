...
- [x] Same question via MCP client and web agent selects the same datasource/model and produces an equivalent LogicalQuery (parity harness). — T13 complete (commits `081d0ffa`, `cb747e57`): 8-case bilingual fixed question set run through the real MCP server and the real web-agent Runtime/ProviderPlanner/WebTools against a shared deterministic fake backend, comparing model/datasource selection and LogicalQuery equivalence via `query.ComputeFingerprint`. Caveat disclosed in-code: the fixture's `handleListModels` returns exactly one model per datasource, so the harness's 0-divergence result for model selection is a structural fact of the fixture, not proof the two paths agree when a real model choice exists.
...
- [~] Phase 3 — T13–T15: T13 (parity harness) and T14's docs + full local gate scope complete; T14's dev-cluster deploy/SSE-gateway smoke test and T15 (staged rollout: dark → allowlist → beta → default) not started — prod upgrade needs explicit go-ahead.
...
- Phase 0-2 (T1-T12, backend + frontend) is now fully complete on `dev`.
- T13 complete (commits `081d0ffa`, `cb747e57`): `internal/agent/parity` 8-case bilingual parity harness runs the real MCP server and the real web-agent `Runtime`/`ProviderPlanner`/`WebTools` against one shared deterministic fake backend, comparing datasource/model selection and LogicalQuery equivalence (`query.ComputeFingerprint`) — zero divergence across all 8 cases. Disclosed limitation: the fixture returns exactly one model per datasource, so the model-selection 0-divergence result is structural, not proof of parity when a real model choice exists; the comparison logic itself is separately unit-tested with synthetic multi-model data. Dev tooling only, not wired into `make eval-regression` (stub-provider harness drives the legacy pipeline, not the agent planner loop — same documented gap as the NATS agent runner).
- T14 (docs + full local gate scope) complete: audited `docs/configuration.md`'s Web Agent Mode row against current `WebAgentConfig`/`normalizeWebAgentConfig`/`validateWebAgentConfig` — two "Primary Load / Override Point" cells pointed at a nonexistent `internal/agent.Runtime`; corrected to the real call sites in `internal/http/handlers/ai_agent_chat.go` (`normalizeWebAgentConfig`, `webAgentRunContext`, `acquireWebAgentSlot`, `workspaceAllowed`) and documented the concurrency-slot TTL / gateway-timeout relationship. Added `CONTEXT.md` touchpoints (Product boundary row, two glossary terms, a Key flows entry, a Where-to-look-in-code row) since it already documents the sibling Agentic query runner and MCP gateway subsystems at this granularity. Full local gate: `make precommit` (gofmt+lint-go+test-go+check-frontend) green; `make eval-regression` green; `deadcode -test $(go list ./... | grep -v '/frontend')` found 2 pre-existing unreachable funcs unrelated to this feature (`eval.SaveRunSnapshot`, `mail_test_helpers.mockSMTPServer.ReceivedBodies`, both pre-dating this branch) — left untouched, out of scope; `gograph_review --uncommitted` ran clean (no uncommitted Go changes at review time — this task's changes are docs-only); `make verify-main` failed only at the `govulncheck` step on two Go **stdlib** CVEs (GO-2026-5856, GO-2026-4970) fixed in go1.26.5 — the local/CI toolchain is pinned to go1.26.4 via `go.mod`'s `go 1.26.4` directive, unrelated to and pre-dating Web Agent Mode; every other verify-main sub-step (vet, lint-go, lint-locale-literals, test-go, coverage-gate, eval-regression) passed, and `check-frontend`/`helm-lint`/`helm-template`/`semgrep-scan` (0 findings) were also confirmed green standalone. `make helm-lint`, `make helm-template`, `make helm-assert-web-agent-route`, and `make helm-assert-agent` all pass. Toolchain bump (`go.mod` `go 1.26.4` → `1.26.5`) is a repo-wide decision out of scope for this dispatch — flagged, not fixed.
- T14's dev-cluster deploy + SSE smoke test: **deferred, not done.** Checked cluster state first: the `prag` cluster has exactly one namespace/release (`biqly`, rev 82), currently running `values-prod.yaml`'s pinned image tags; `values-dev.yaml` has no tag overrides (inherits base `tag: latest`), so a "dev" deploy would revert the live release's images in place — there's no isolated dev sandbox. Flagged this to the user; they chose to skip the cluster deploy. Fell back to a local smoke test attempt (`make watch SVC="api auth mail"` + `make dev-frontend`, `BI_WEB_AGENT_ENABLED=true` in gitignored `.env.dev`) — stack came up clean, but no local dev login credentials or PAT were available to actually drive a request through the browser or curl, and the user chose to skip providing/minting one too. Local processes stopped, `.env.dev` reverted — no lasting changes. **Net effect: the full live-request SSE flow has not been exercised end-to-end by anyone in this session** — only in-process Go tests and frontend component tests have verified it. Recommend a manual pass (with real credentials) before any rollout decision.
- Remaining work: T14's dev-cluster deploy + SSE smoke test (deferred, see above) and T15 staged rollout (prod steps require explicit go-ahead per the plan).
