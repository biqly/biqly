...
**Status:** in progress — Phase 0-2 complete (T1-T12, backend + frontend); Phase 3's T13 (parity harness) and T14 (docs + full local gate) also complete. T15 (staged rollout) not started; prod rollout steps require explicit go-ahead.
...
- Remaining work: T14's dev-cluster `helm upgrade` deploy + gateway SSE smoke test (handled separately, mutates a live shared cluster) and T15 staged rollout (prod steps require explicit go-ahead per the plan).
