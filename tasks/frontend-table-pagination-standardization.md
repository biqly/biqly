...
Bu faz bağımsız iş paketleri halindedir; durum (2026-06-12):

- [x] **7.1 Button:** `frontend/src/components/ui/Button.tsx` (`variant: primary|secondary|ghost|danger`, `size: sm|md`, `autoWidth`, native button props; mevcut `btn btn-*` sınıflarını **aynen** üretir — CSS değişmedi). Pilotlar: `ui/EmptyState` action butonu ve `ConfirmedQueriesPanel` deactivate. **Karar:** `loading` prop'u EKLENMEDİ (mevcut kalıp `saving ? t('saving') : t('save')` metin değişimi — bileşen prop'u karşılamaz, spekülatif olurdu); `admin-btn-*` ailesi ayrı görsel dil, bilinçli kapsam dışı. Yeni-kod kuralları `tasks/lessons.md → Frontend Conventions → Shared List/Table Building Blocks`'a yazıldı. Kalan ~170 kullanım fırsatçı migrasyon.
- [x] **7.2 FormField:** `frontend/src/components/ui/FormField.tsx` (label + input + `aria-invalid`/`aria-describedby`'lı hata satırı, `useId`; mevcut `.form-field`/`.form-label`/`.input` sınıf ailesi + yeni `styles/form-field.css` yalnızca hata satırı için). Pilot: `WorkspacesPanel` create formu — inline-style'lı label/input/buton üçlüsü standart sınıflara geçti (bilinçli görsel normalizasyon; ölü `inputStyle`/`btnPrimary` sabitleri silindi).
- [ ] **7.3 Modal konsolidasyonu — ERTELENDİ (envanter düzeltildi):** grep (`createPortal|role="dialog"`) gerçek listeyi 5 dosyaya indirdi (ajan tahmini ~12 abartılıymış): `admin/userList/InviteUserModal.tsx`, `metadata/MetadataDescribeModal.tsx`, `metadata/MetadataBulkDescribeModal.tsx`, `tableBrowser/TableBrowserCellValue.tsx` (+ `ui/CommandPalette.tsx` — bilinçli özel, kapsam dışı). Her biri ayrı odaklı dokunuş istiyor; fırsatçı migrasyon kuralıyla bırakıldı.
- [x] **7.4 i18n temizliği (tablo kapsamı):** `ActiveUsersTab`/`InvitationsTab` (`t` + `locale` propları kalktı → `useT()`/`useLocale()`; `UserListPage`'den geçişler silindi), `TimeGrainsTable`/`TimeGrainsEditModal` (+ `TimeGrains.tsx` çağrıları). `ui/Pagination.tsx` anahtarları `common.pagination.{range_of_total,page_number}`'a taşındı (EN+TR aynı PR'da); yalnızca Pagination'ın kullandığı `table_browser.page_number` her iki locale'den silindi; `table_browser.range_of_total` tableBrowser ekranlarında kalmaya devam ediyor (kendi alanı). **Kalan 16 `t`-prop dosyası** (queryBuilder/*Step ×6, auth ×2, evaluation ×3, savedQuestions ×2, SidebarConversationItem, PlatformSettingsPanel) fırsatçı kurala bırakıldı — lessons.md kuralı yeni dosyalarda prop'u yasaklıyor.
- [x] **7.5 Stil temizliği (kısmi):** `ui/Pagination.tsx` inline stilleri `styles/pagination.css`'e taşındı (`.table-pagination`, `.table-pagination__label` BEM blokları). `ActiveUsersTab` avatar inline stilleri ve `index.css` bölme işi kapsam dışı bırakıldı (ayrı, görsel-riskli iş).
- [x] **Ek UX düzeltmesi — sağ-alt loading pili (2026-06-12):** route-level Suspense fallback'i ile ekranların kendi `LoadingScreen`'leri aynı sabit köşeye **üst üste** pil basıyordu (iç içe görünüm + ekran okuyucuya çift `role="status"` anonsu). Çözüm: `ui/loadingScreenRegistry.ts` — kaç instance mount olursa olsun pili yalnızca ilk mount edilen "owner" render eder, owner unmount olunca sahiplik kesintisiz devredilir; ~250ms görünme gecikmesi (hızlı yüklemelerde pil hiç parlamaz), ardışık fazlar arasında (`HANDOFF_WINDOW_MS`) gecikme atlanır (tek kesintisiz gösterge), fade/slide giriş animasyonu + `prefers-reduced-motion`'da yalnızca fade. Registry testli (`loadingScreenRegistry.test.ts`).
- [x] **7.6 URL state (OQ-2 ONAYLANDI, 2026-06-12):** `usePaginatedList`'e `syncToUrl?: string` opsiyonu eklendi. Tasarım: URL **source of truth** — sayfa değeri `useQueryParam` üzerinden okunur/yazılır (state çiftlenmez), `history.replaceState` kullanıldığından geri-tuşu kirlenmez, sayfa 1 URL'den silinir (temiz adres çubuğu), geçersiz/elle bozulmuş param sayfa 1'e düşer (`parsePageParam`/`pageParamValue` pure + testli). `resetPageKey` reset'i sayfa-1'e dönünce param otomatik silinir. Opsiyon verilmediğinde davranış birebir eskisi gibi (internal state). **Aktivasyon:** `AuditLogPanel` (`?auditPage=N`) ve `QueryHistory` (`?page=N`) pilotları; diğer ekranlar tek satırlık opt-in (`syncToUrl: '<paramAdı>'` — ekran içinde benzersiz ad seçilir; UserListPage gibi çift-listeli ekranlarda iki farklı ad gerekir).
...
- ~~OQ-2~~ **KAPATILDI (Faz 7.6, onaylandı):** Sayfa numarası `syncToUrl` opsiyonuyla URL'e yazılıyor (pilot: AuditLogPanel, QueryHistory); kalan ekranlar tek satırlık opt-in. Filtre değerlerinin URL'e yazılması ayrı bir karar olarak açık (gerekirse aynı pattern'le eklenir).
