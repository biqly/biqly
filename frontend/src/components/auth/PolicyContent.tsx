...
export function TermsOfUseEN(): JSX.Element {
  return (
    <div style={{ lineHeight: '1.6', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
      ...
      <ul>
        <li><strong>AI Output Limitations:</strong> You acknowledge that ABI uses Large Language Models (LLMs) to generate queries. AI-generated queries can occasionally be inaccurate, incomplete, or contain hallucinations. You are responsible for reviewing and verifying queries before relying on them.</li>
        <li><strong>ReadOnly & Timeout Safety:</strong> To protect your database infrastructure, ABI automatically enforces strict security limits: read-only connection verification, maximum execution timeouts (default 30 seconds), and maximum row limits (default 10,000 rows). You must not attempt to bypass these safety constraints.</li>
        <li><strong>Security Rules:</strong> Direct modification commands (e.g., INSERT, UPDATE, DELETE, DROP, ALTER) are strictly rejected by the semantic compiler and the read-only checker.</li>
      </ul>
...
export function TermsOfUseTR(): JSX.Element {
  return (
    <div style={{ lineHeight: '1.6', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
      ...
      <p>
        Yasaların izin verdiği azami ölçüde ABI; kâr kaybı, veri kaybı, veri tabanı kesintisi, performans düşüşü veya Hizmet kullanımınızdan kaynaklanan güvenlik ihlalleri dahil ancak bunlarla sınırlı olmamak üzere hiçbir dolaylı, arızi, özel veya cezai zarardan sorumlu tutulamaz.
      </p>
    </div>
  )
}
...
export function PrivacyPolicyEN(): JSX.Element {
  return (
    <div style={{ lineHeight: '1.6', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
      ...
      <ul>
        <li><strong>Account Information:</strong> We collect your name, email address, and authentication credentials (such as Passkey public keys and MFA metadata) when you register.</li>
        <li><strong>Metadata & Schemas:</strong> To build accurate context for AI query generation, we introspect and store database schema metadata (table names, column types, foreign keys, and descriptions).</li>
        <li><strong>Audit & Query History:</strong> We log query logs containing execution status, performance metrics, duration, query fingerprints, and generated SQL.</li>
      </ul>
...
export function PrivacyPolicyTR(): JSX.Element {
  return (
    <div style={{ lineHeight: '1.6', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
      ...
      <ul>
        <li><strong>Hesap Bilgileri:</strong> Kayıt olduğunuzda adınızı, e-posta adresinizi ve kimlik doğrulama bilgilerini (Passkey açık anahtarları ve MFA meta verileri gibi) toplarız.</li>
        <li><strong>Meta Veri ve Şemalar:</strong> Yapay zeka sorgu üretimi için doğru bağlam oluşturmak amacıyla, veri tabanı şeması meta verilerini (tablo adları, sütun tipleri, yabancı anahtarlar ve açıklamalar) inceler ve depolarız.</li>
        <li><strong>Denetim ve Sorgu Geçmişi:</strong> Çalışma durumu, performans ölçümleri, süre, sorgu parmak izleri ve üretilen SQL sorgularını içeren denetim günlüklerini kaydederiz.</li>
      </ul>
