...
export function scorePassword(password: string): number {
  ...
  for (const ch of password) {
    if (/[A-ZÇĞİÖŞÜ]/.test(ch)) hasUpper = true
    else if (/[a-zçğıöşü]/.test(ch)) hasLower = true
    else if (/[0-9]/.test(ch)) hasDigit = true
    else hasSpecial = true
  }
  for (const c of [hasUpper, hasLower, hasDigit, hasSpecial]) if (c) classes++
  ...
  if (hasRunOrSequence(password)) score--
  if (isObviouslyWeak(password)) score -= 2
...
const CLIENT_WEAK_PREFIXES = [
  ...
  'monkey', '12345', '123456', 'iloveyou', 'biqly', 'abi', 'master',
]
...
export function rulesFor(password: string, policy: PasswordPolicy, labels: RuleLabels): RuleStatus[] {
  const length = [...password].length
  const rules: RuleStatus[] = [
    { key: 'length', label: labels.length, ok: length >= policy.min_length },
  ]
  if (policy.require_upper) rules.push({ key: 'upper', label: labels.upper, ok: /[A-ZÇĞİÖŞÜ]/.test(password) })
  if (policy.require_lower) rules.push({ key: 'lower', label: labels.lower, ok: /[a-zçğıöşü]/.test(password) })
  if (policy.require_digit) rules.push({ key: 'digit', label: labels.digit, ok: /[0-9]/.test(password) })
  if (policy.require_special) rules.push({ key: 'special', label: labels.special, ok: /[^A-Za-z0-9ÇĞİÖŞÜçğıöşü]/.test(password) })
  return rules
}
