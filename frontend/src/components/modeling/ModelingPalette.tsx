...
type EntityImpact = { joins: number; dims: number; metrics: number }
...
export function ModelingPalette({
  ...
  return (
    <aside className={`modeling-palette ${open ? '' : 'modeling-side--collapsed'}`} aria-label={t('modeling.model_summary_aria')}>
      <button
        type="button"
      ...
      <div className="modeling-side-body">
        ...
        <div className="modeling-stat-grid">
          <div><strong>{usedTableCount}</strong><span>{t('modeling.tab_short_tables')}</span></div>
          <div><strong>{joins.length}</strong><span>{t('modeling.tab_short_rel')}</span></div>
          <div><strong>{dims.length}</strong><span>{t('modeling.tab_short_dim')}</span></div>
          <div><strong>{metrics.length}</strong><span>{t('modeling.tab_short_metric')}</span></div>
        </div>
        ...
        <div className="modeling-tabs">
          {(['tables', 'joins', 'dimensions', 'metrics'] as const).map((tab) => (
            ...
            >
              {t(tab === 'joins' ? 'modeling.tab_short_rel' : `modeling.tab_short_${tab === 'dimensions' ? 'dim' : tab === 'metrics' ? 'metric' : 'tables'}`)}
            </button>
          ))}
        </div>
        ...
        <div className="modeling-tab-content">
          {activeTab === 'tables' && (
            <div className="modeling-join-list">
              ...
              {Array.from(new Set(tables.map((table) => table.schema_name))).sort().map((schemaName) => {
              ...
              {tables.length === 0 ? (
                ...
                const isOnCanvas = tableCards.some((card) => tableKey(card.schema_name, card.table_name) === key)
                const isBase = model ? key === tableKey(model.base_schema, model.base_table) : false
                const impact = model ? tableImpact(table.schema_name, table.table_name) : { joins: 0, dims: 0, metrics: 0 }
                const inModel = isBase || impact.joins > 0 || impact.dims > 0 || impact.metrics > 0
                return (
                  <div className={`modeling-join-pill ${isOnCanvas ? 'modeling-join-pill--active' : ''}`} key={table.id}>
                    <div className="modeling-join-pill-header">
                      <strong>
                      ...
                      <span className="modeling-pill-actions">
                        <button className="modeling-rename-btn" onClick={() => onRenameTable(table)} title={t('modeling.edit_display_name_title')}>✎</button>
                        {!isBase && inModel && <button className="modeling-rename-btn" onClick={() => onMakeBase(table.schema_name, table.table_name)} title={t('modeling.make_base_title')}>★</button>}
                        {isOnCanvas && !isBase && <button className="modeling-delete-btn" onClick={() => onRemoveTable(table.schema_name, table.table_name)} title={inModel ? t('modeling.remove_from_model_title') : t('modeling.hide_from_canvas_title')}>×</button>}
                        {!isOnCanvas && <button className="modeling-add-btn" onClick={() => onToggleTableVisibility(table.schema_name, table.table_name, true)} title={t('modeling.show_on_canvas_title')}>+</button>}
                        {isBase && <button className="modeling-delete-btn" onClick={onOpenBaseSwap} title={t('modeling.change_base_title')}>×</button>}
                      </span>
                    </div>
                    <span className="modeling-join-meta">{table.schema_name}.{table.table_name}</span>
          ...
          {activeTab === 'joins' && (
            <div className="modeling-join-list">
              ...
              {joins.length === 0 ? <p className="modeling-empty">{t('modeling.no_relationships')}</p> : joins.map((join) => (
                <div
                ...
                >
                  <div className="modeling-join-pill-header">
                    ...
                    <button className="modeling-delete-btn" onClick={() => onDeleteJoin(join.id)} title={t('modeling.delete_relationship_title')}>×</button>
                  </div>
                ...
                <>
                  ...
                  {suggestedJoins.map((join, index) => (
                    <div className="modeling-join-pill modeling-join-pill--suggested" key={index}>
                      <div className="modeling-join-pill-header">
                        <strong>{join.from_table}.{join.from_column} → {join.to_table}.{join.to_column}</strong>
                        <button className="modeling-add-btn" onClick={() => onAddSuggestedJoin(join)} title={t('common.add')}>+</button>
                      </div>
                    </div>
                  ))}
                </>
                ...
                <>
                  ...
                  {inactiveJoins.map((join) => (
                    <div className="modeling-join-pill modeling-join-pill--suggested" key={join.id}>
                      <div className="modeling-join-pill-header">
                        <strong>{join.name}</strong>
                        <button className="modeling-add-btn" onClick={() => onReactivateJoin(join)} title={t('modeling.reactivate_title')}>+</button>
                      </div>
                      <span>{join.from_table}.{join.from_column} → {join.to_table}.{join.to_column}</span>
                    </div>
                  ))}
          ...
          {activeTab === 'dimensions' && (
            <div className="modeling-join-list">
              ...
              {dims.length === 0 ? <p className="modeling-empty">{t('modeling.no_dimensions')}</p> : dims.map((dimension) => (
              ...
              {inactiveDims.length > 0 && (
                <>
                  ...
                  {inactiveDims.map((dimension) => (
                    <div className="modeling-join-pill modeling-join-pill--suggested" key={dimension.id}>
                      <div className="modeling-join-pill-header">
                        <strong>{dimension.label || dimension.name}</strong>
                        <button className="modeling-add-btn" onClick={() => onReactivateDimension(dimension)} title={t('modeling.reactivate_title')}>+</button>
                      </div>
                      <span>{dimension.column_ref}</span>
                    </div>
                  ))}
          ...
          {activeTab === 'metrics' && (
            <div className="modeling-join-list">
              <div className="modeling-section-header" style={{ justifyContent: 'center' }}>
                <button className="btn btn-sm btn-primary" type="button" onClick={onOpenAddMetric} disabled={!model} style={{ width: '100%' }}>
                  {t('modeling.add_metric_btn')}
                </button>
              </div>
              {metrics.length === 0 ? <p className="modeling-empty">{t('modeling.no_metrics')}</p> : metrics.map((metric) => (
                <div className="modeling-join-pill" key={metric.id}>
                  <div className="modeling-join-pill-header">
                    <strong>{metric.label || metric.name}</strong>
                    <span className="modeling-pill-actions">
                      <button className="modeling-rename-btn" onClick={() => onRenameMetric(metric)} title={t('modeling.edit_display_name_title')}>✎</button>
                      <button className="modeling-delete-btn" onClick={() => onDeleteMetric(metric.id)} title={t('modeling.delete_metric_title')}>×</button>
                    </span>
                  </div>
                ...
                <>
                  ...
                  {inactiveMetrics.map((metric) => (
                    <div className="modeling-join-pill modeling-join-pill--suggested" key={metric.id}>
                      <div className="modeling-join-pill-header">
                        <strong>{metric.label || metric.name}</strong>
                        <button className="modeling-add-btn" onClick={() => onReactivateMetric(metric)} title={t('modeling.reactivate_title')}>+</button>
                      </div>
                      <span>{metric.expression}</span>
                    </div>
                  ))}
