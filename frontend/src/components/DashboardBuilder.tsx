...
export default function DashboardBuilder({ dashboardId, onBack }: DashboardBuilderProps) {
  ...
  const handleOpenConfig = (w: Widget) => {
    ...
    if (w.saved_query_id) {
      ...
      if (w.logical_query && w.logical_query.datasource_id) {
    ...
    <div className="page-stack" style={{ position: 'relative' }}>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <button type="button" className="btn btn-secondary btn-sm" onClick={onBack} style={{ marginBottom: '0.5rem' }}>
              ← Back to Dashboards
            </button>
            <h2 style={{ fontSize: '1.8rem', fontWeight: 700 }}>{dashboard?.name}</h2>
            {dashboard?.description && <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '0.2rem' }}>{dashboard.description}</p>}
          </div>
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            {isEditMode ? (
              <>
                <button type="button" className="btn btn-secondary" onClick={() => { setIsEditMode(false); fetchDashboard(); }}>
                  Cancel
                </button>
                <button type="button" className="btn btn-primary" onClick={handleSaveLayout} disabled={!isDirty}>
                  💾 Save Dashboard
                </button>
              </>
            ) : (
        ...
        {/* Toolbox */}
        {isEditMode && (
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem', padding: '1rem 0 0 0', borderTop: '1px solid var(--border)' }}>
            <span style={{ alignSelf: 'center', fontWeight: 600, color: 'var(--text-muted)', fontSize: '0.9rem' }}>Add Widget:</span>
            <button type="button" className="btn btn-secondary btn-sm" onClick={() => handleAddWidget('kpi')}>➕ KPI</button>
            <button type="button" className="btn btn-secondary btn-sm" onClick={() => handleAddWidget('chart')}>➕ Chart</button>
            <button type="button" className="btn btn-secondary btn-sm" onClick={() => handleAddWidget('table')}>➕ Table</button>
            <button type="button" className="btn btn-secondary btn-sm" onClick={() => handleAddWidget('text')}>➕ Text</button>
          </div>
        )}
      </div>
      ...
      {/* Widgets Grid */}
      {widgets.length === 0 ? (
        <div className="card" style={{ padding: '6rem 2rem', textAlign: 'center', border: '2px dashed var(--border)' }}>
          <p style={{ color: 'var(--text-muted)' }}>This dashboard is empty.</p>
          {isEditMode && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginTop: '1rem' }}>
              <button type="button" className="btn btn-secondary" onClick={() => handleAddWidget('chart')}>Add Chart</button>
              <button type="button" className="btn btn-secondary" onClick={() => handleAddWidget('kpi')}>Add KPI</button>
            </div>
          )}
        </div>
      ) : (
        ...
        >
          {widgets.map((widget, idx) => (
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem', borderBottom: '1px solid var(--border-light, #f1f5f9)', paddingBottom: '0.4rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setWidgets(widgets.map((w) => (w.id === widget.id ? { ...w, w: newW } : w)))
                        setIsDirty(true)
                      }}
                      title="Shrink Width"
                    >
                    ...
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{widget.w || 6}</span>
                    <button
                      type="button"
                      className="btn btn-secondary btn-sm"
                      style={{ padding: '0.1rem 0.3rem', fontSize: '0.75rem' }}
        ...
        <div className="modal-backdrop">
          <div className="modal-content" style={{ maxWidth: '600px', width: '90%' }}>
            <div className="modal-header">
              ...
              <button type="button" className="modal-close" onClick={() => setIsConfigModalOpen(false)}>✕</button>
            </div>
            <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxHeight: '70vh', overflowY: 'auto' }}>
              <div className="form-field">
                <label className="form-label" htmlFor="w-title">Widget Title</label>
                <input id="w-title" type="text" value={configTitle} onChange={(e) => setConfigTitle(e.target.value)} required />
              </div>

              <div className="form-row">
                <div className="form-field" style={{ flex: 1 }}>
                  <label className="form-label" htmlFor="w-width">Width Grid Span (2-12 Columns)</label>
                  <Select
                    id="w-width"
                    value={configWidth.toString()}
                    onChange={(val) => setConfigWidth(parseInt(val, 10))}
                    options={Array.from({ length: 11 }, (_, i) => ({ value: (i + 2).toString(), label: `${i + 2} Columns` }))}
                  />
                </div>
                <div className="form-field" style={{ flex: 1 }}>
                  <label className="form-label" htmlFor="w-height">Height Class</label>
                  <Select
                    id="w-height"
                    value={configHeight}
              ...
              <div className="form-field">
                <label className="form-label" htmlFor="w-type">Widget Type</label>
                <Select
                  id="w-type"
                  value={configType}
              ...
              {configType === 'text' ? (
                <div className="form-field">
                  <label className="form-label" htmlFor="w-content">Text Content</label>
                  <textarea
                    id="w-content"
                    value={configContent}
                ...
                <>
                  <div style={{ padding: '0.8rem', border: '1px solid var(--border)', borderRadius: '0.5rem', background: 'var(--bg-card-raised)' }}>
                    <h4 style={{ margin: '0 0 0.8rem 0', fontSize: '0.9rem', fontWeight: 600 }}>Link Saved Query Data Source</h4>

                    <div className="form-row" style={{ gap: '0.75rem', marginBottom: '0.8rem' }}>
                      <div className="form-field" style={{ flex: 1 }}>
                        <label className="form-label" htmlFor="w-ds">Datasource</label>
                        <Select
                          id="w-ds"
                          value={selDatasourceId}
                          onChange={(val) => { setSelDatasourceId(val); setSelModelId(''); setSelQuestionId(''); }}
                          options={datasources.map((d) => ({ value: d.id, label: d.name }))}
                        />
                      </div>
                      <div className="form-field" style={{ flex: 1 }}>
                        <label className="form-label" htmlFor="w-model">Model</label>
                        <Select
                          id="w-model"
                          value={selModelId}
                          onChange={(val) => { setSelModelId(val); setSelQuestionId(''); }}
                          options={[
                    ...
                    <div className="form-field">
                      <label className="form-label" htmlFor="w-question">Saved Question (Query)</label>
                      <Select
                        id="w-question"
                        value={selQuestionId}
                        ...
                        options={[
                          ...
                          ...savedQuestions.map((q) => ({ value: q.id, label: q.name || q.question }))
                        ]}
                      />
                    </div>
                  </div>
                  ...
                  {selQuestionId && availableColumns.length > 0 && (
                    <div style={{ padding: '0.8rem', border: '1px solid var(--border)', borderRadius: '0.5rem', marginTop: '0.5rem' }}>
                      <h4 style={{ margin: '0 0 0.8rem 0', fontSize: '0.9rem', fontWeight: 600 }}>Map Fields / Columns</h4>

                      {configType === 'kpi' && (
                        <div className="form-field">
                          <label className="form-label" htmlFor="w-kpi-col">KPI Metric Column</label>
                          <Select
                            id="w-kpi-col"
                            value={valueColumn}
                      ...
                      {configType === 'chart' && (
                        <div style={{ display: 'grid', gap: '1rem' }}>
                          <div className="form-field">
                            <label className="form-label" htmlFor="w-chart-type">Chart Visualization Type</label>
                            <Select
                              id="w-chart-type"
                          ...
                          <div className="form-row" style={{ gap: '0.75rem' }}>
                            <div className="form-field" style={{ flex: 1 }}>
                              <label className="form-label" htmlFor="w-xaxis">X-Axis Category Column</label>
                              <Select
                                id="w-xaxis"
                                value={xAxisColumn}
                            ...
                            <div className="form-field" style={{ flex: 1 }}>
                              <label className="form-label">Y-Axis Metric Series (Select multiple)</label>
                              <div style={{ maxHeight: '100px', overflowY: 'auto', border: '1px solid var(--border)', padding: '0.4rem', borderRadius: '0.4rem' }}>
                                {availableColumns.map((col) => {
                                  const checked = yAxisColumns.includes(col)
                                  return (
                                    <label key={col} style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', fontSize: '0.8rem', padding: '0.1rem 0', cursor: 'pointer' }}>
                                      <input
                                        type="checkbox"
                                        checked={checked}
                                        onChange={() => {
                      ...
                      {configType === 'table' && (
                        <div className="form-field">
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.8rem', border: '1px solid var(--border)', padding: '0.6rem', borderRadius: '0.4rem' }}>
                            {availableColumns.map((col) => {
                              const checked = visibleColumns.includes(col)
                              return (
                                <label key={col} style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', fontSize: '0.8rem', cursor: 'pointer' }}>
                                  <input
                                    type="checkbox"
                                    checked={checked}
                                    onChange={() => {
            ...
            <div className="modal-footer">
              ...
              <button type="button" className="btn btn-primary" onClick={handleSaveConfig} disabled={configType !== 'text' && !selQuestionId}>
                Apply Configuration
              </button>
            </div>
          </div>
...
function WidgetRenderer({ widget }: { widget: Widget }) {
  ...
  if (widget.type === 'text') {
    return (
      <div style={{ padding: '0.5rem', whiteSpace: 'pre-wrap', overflow: 'auto', height: '100%', fontSize: '0.95rem', lineHeight: '1.5' }}>
  ...
  if (!widget.saved_query_id) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: '120px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: '120px' }}>
        <span className="status-dot" style={{ background: 'var(--accent)', animation: 'pulse 1.5s infinite' }} />
  ...
  if (error) {
    return (
      <div style={{ color: 'var(--error)', padding: '1rem', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem' }}>
        ⚠️ Error: {error}
      </div>
    )
  }
  ...
  if (widget.type === 'kpi') {
    ...
    const val = data[0] && data[0][valCol] !== undefined ? data[0][valCol] : 'N/A'
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%', minHeight: '100px', padding: '0.5rem' }}>
        <span style={{ fontSize: '2.2rem', fontWeight: 800, color: 'var(--accent)', wordBreak: 'break-all' }}>
          {formattedVal}
        </span>
        ...
        <ResponsiveContainer width="100%" height="100%">
          {widget.chart_type === 'bar' ? (
            ...
            <LineChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              <Legend />
              {yKeys.map((key, idx) => (
                <Line key={key} type="monotone" dataKey={key} stroke={chartColor(idx)} strokeWidth={2} dot={{ r: 3 }} />
              ))}
            </LineChart>
          )}
        </ResponsiveContainer>
