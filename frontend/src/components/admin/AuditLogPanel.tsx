  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start' }}>
        ...
        <div style={{ color: '#6b7280', fontSize: 13 }}>{t('admin.audit.count', { count: entries.length })}</div>
      </div>
      ...
      {!loading && !error && entries.length === 0 && <div style={{ color: '#6b7280' }}>{t('admin.audit.empty')}</div>}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 980 }}>
            <thead>
              <tr style={{ textAlign: 'left', background: '#f9fafb' }}>
                <th style={th}>{t('admin.audit.time')}</th>
                <th style={th}>{t('admin.audit.action')}</th>
                <th style={th}>{t('admin.fields.user')}</th>
                <th style={th}>{t('admin.audit.resource')}</th>
                <th style={th}>IP</th>
