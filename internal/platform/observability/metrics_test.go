...
func TestMetricsRecordTier2(t *testing.T) {
	...
	m.RecordEmbeddingCacheMiss()
	m.RecordAmbiguityLLMTierYield("found")
	m.RecordAmbiguityLLMTierYield("empty")
	m.RecordAmbiguityLLMTierYield("unbounded_random_outcome")
	...
	assertMetric(t, m.embeddingCacheHits, 1, "biqly_embedding_cache_hits_total")
	assertMetric(t, m.embeddingCacheMisses, 2, "biqly_embedding_cache_misses_total")
	assertMetric(t, m.ambiguityLLMTierYield.WithLabelValues("found"), 1, "biqly_ambiguity_llm_tier_yield_total{outcome=found}")
	assertMetric(t, m.ambiguityLLMTierYield.WithLabelValues("empty"), 1, "biqly_ambiguity_llm_tier_yield_total{outcome=empty}")
	assertMetric(t, m.ambiguityLLMTierYield.WithLabelValues("error"), 1, "biqly_ambiguity_llm_tier_yield_total{outcome=error}")
