...
// scanTableEmbedding keeps a single-entry scan for tests that expect one row.
func scanTableEmbedding(s platformdb.Scanner) (TableEmbedding, error) {
	entries, err := scanTableEmbeddingRow(s)
	if err != nil {
		return TableEmbedding{}, err
	}
	if len(entries) == 0 {
		return TableEmbedding{}, errCorruptEmbedding
	}
	return entries[0], nil
}

func scanColumnEmbedding(s platformdb.Scanner) (ColumnEmbedding, error) {
	entries, err := scanColumnEmbeddingRow(s)
	if err != nil {
		return ColumnEmbedding{}, err
	}
	if len(entries) == 0 {
		return ColumnEmbedding{}, errCorruptEmbedding
	}
	return entries[0], nil
}
