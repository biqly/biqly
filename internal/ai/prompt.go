...
func (b *PromptBuilder) Build(question string, model *semantic.SemanticModel, maxPromptRunes int, examples []FewShotExample, samples []TableSample, priorTurns []ConversationTurn, deniedFields []string) string {
	...
	write("- For grouping or listing by calendar period, use the matching time-grain dimension: `*_day` for \"daily\" / \"by day\" / \"günlük\", `*_month` for \"monthly\" / \"by month\" / \"aylık\", `*_quarter` for \"quarterly\" / \"by quarter\" / \"çeyreklik\", `*_year` for \"yearly\" / \"yıllık\" — not the raw timestamp/date column. Always include the chosen time-grain dimension in BOTH `select` AND `group_by` so the result shows the period column alongside the metrics. If the user asks for a year filter PLUS a finer grain (e.g. \"2026 yılında günlük tweet sayısı\"), filter on `*_year` and group by `*_day` / `*_month`.\n")
