...
// SeedFromEnv inserts a provider + default models derived from the BI_AI_*
// fallback config when the ai_providers table is empty. It is idempotent: once
// any provider exists the seed is skipped. Returns whether a seed was written.
func (s *ProviderStore) SeedFromEnv(ctx context.Context) (bool, error) {
	var count int
	if err := s.db.QueryRowContext(ctx, `SELECT count(*) FROM ai_providers`).Scan(&count); err != nil {
		return false, fmt.Errorf("count ai providers: %w", err)
	}
	if count > 0 {
		return false, nil
	}
	...
	c := s.fallback
	providerType := strings.ToLower(strings.TrimSpace(c.Provider))
	if providerType == "" {
		providerType = "openai"
	}
	...
	if baseURL == "" {
		baseURL = DefaultBaseURLForType(providerType)
	}

	providerID, err := s.CreateProvider(ctx, CreateProviderInput{
		Name:               "Default (env)",
	...
	// query + describe + judge use the base/query model; embedding & translation
	// only seed when their env vars are configured.
	queryCfg := c.EffectiveQueryConfig()
	s.seedModel(ctx, providerID, PurposeQuery, queryCfg.Model, c, queryCfg)
	s.seedModel(ctx, providerID, PurposeDescribe, c.Model, c, c)
	s.seedModel(ctx, providerID, PurposeJudge, c.Model, c, c)

	if strings.TrimSpace(c.EmbeddingModel) != "" {
		if embProviderID, ok := s.seedEmbeddingProvider(ctx, c, providerType, providerID); ok {
			_, _ = s.CreateModel(ctx, CreateModelInput{
				ProviderID:  embProviderID,
				ModelID:     strings.TrimSpace(c.EmbeddingModel),
				DisplayName: strings.TrimSpace(c.EmbeddingModel),
				Purpose:     string(PurposeEmbedding),
				IsDefault:   true,
			})
		}
	}
	if strings.TrimSpace(c.TranslationModel) != "" {
		_, _ = s.CreateModel(ctx, CreateModelInput{
			...
			ModelID:     strings.TrimSpace(c.TranslationModel),
			DisplayName: strings.TrimSpace(c.TranslationModel),
			Purpose:     string(PurposeTranslation),
			MaxTokens:   c.MaxTokens,
			IsDefault:   true,
		})
	}

	slog.Info("seeded ai_providers from environment", "provider_type", providerType, "base_url", baseURL)
	return true, nil
}

// seedEmbeddingProvider reuses the base provider when the embedding endpoint
// matches; otherwise it creates a dedicated provider row for embeddings.
func (s *ProviderStore) seedEmbeddingProvider(ctx context.Context, c config.AIConfig, baseType, baseProviderID string) (string, bool) {
	embBase := c.EffectiveEmbeddingBaseURL()
	embKey := c.EffectiveEmbeddingAPIKey()
	if embBase == strings.TrimRight(strings.TrimSpace(c.BaseURL), "/") || embBase == DefaultBaseURLForType(baseType) {
		if embKey == c.APIKey {
			return baseProviderID, true
		}
	}
	id, err := s.CreateProvider(ctx, CreateProviderInput{
		Name:         "Embeddings (env)",
	...
	if err != nil {
		slog.Warn("seed embedding provider failed", "error", err)
		return "", false
	}
	return id, true
}

func (s *ProviderStore) seedModel(ctx context.Context, providerID string, purpose Purpose, modelID string, base, src config.AIConfig) {
	...
	if _, err := s.CreateModel(ctx, CreateModelInput{
		...
		TopP:                src.TopP,
		NumCtx:              src.NumCtx,
		MaxPromptInputRunes: base.MaxPromptInputRunes,
		IsDefault:           true,
	}); err != nil {
		slog.Warn("seed ai model failed", "purpose", purpose, "error", err)
	}
}
