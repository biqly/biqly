...
func main() {
	dsn := flag.String("dsn", os.Getenv("BI_AUTH_DB_DSN"), "Database DSN for auth DB")
	dir := flag.String("dir", "migrations/auth", "Migrations directory")
	flag.Parse()

	if *dsn == "" {
		slog.Error("BI_AUTH_DB_DSN is required")
		os.Exit(1)
	}

	args := flag.Args()
	if len(args) == 0 {
		slog.Error("Usage: auth-migrate [up|down]")
		os.Exit(1)
	}
