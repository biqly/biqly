...
# AI provider API keys are no longer synced to Kubernetes secrets: each
# provider's key is stored encrypted in the ai_providers table and managed via
# the admin UI (Administration → AI Providers).
...
# AI provider/model selection (provider type, model ids, base URLs, per-model
# tuning) lives in the ai_providers / ai_models tables, not in ConfigMaps. Only
# operational knobs are synced below.
kubectl -n "$NS" create configmap biqly-ai-config \
  ...
  --from-literal="BI_AUTH_SERVICE_URL=http://biqly-auth:8889" \
  --from-literal="BI_AI_TRANSLATION_TARGET_LANGUAGE=${BI_AI_TRANSLATION_TARGET_LANGUAGE:-Turkish}" \
