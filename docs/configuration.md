...
| `BI_API_SERVICE_URL` | `""` | `http://eg-gw-http.envoy-gateway-system.svc.cluster.local:80` | No | `internal/config/config.go`, `internal/http/mcp_router.go` | No | Base URL of the API gateway the standalone MCP/AI services forward governed tool calls to. Point it at the in-cluster gateway service (with `BI_API_SERVICE_HOST`) so dispatches don't round-trip through Cloudflare. Empty in the monolith. |
| `BI_API_SERVICE_HOST` | `""` | `abi.il1.nl` | No | `internal/config/config.go`, `internal/http/upstream_proxy.go` | No | Host header override for `BI_API_SERVICE_URL` dispatches. Required when the URL targets the in-cluster gateway service, whose HTTPRoutes match on the public hostname. |
...
| `BI_WEB_AGENT_MAX_STEPS` | `6` | N/A | No | `internal/config/config.go`, `internal/http/handlers/ai_agent_chat.go` (`normalizeWebAgentConfig`, `webAgentRunContext`) | No | Caps web-agent planner tool-call iterations per run; must be between 1 and 6. Re-defaulted to `6` if a zero value ever reaches `normalizeWebAgentConfig` (defensive; `Load()` already validates the range). |
| `BI_WEB_AGENT_MAX_CLARIFICATION_ROUNDS` | `2` | N/A | No | `internal/config/config.go`, `internal/http/handlers/ai_agent_chat.go` (`webAgentRunContext`) | No | Caps web-agent clarification round-trips per run; must be between 0 and 2. |
| `BI_WEB_AGENT_TIMEOUT` | `"120s"` | N/A | No | `internal/config/config.go`, `internal/http/handlers/ai_agent_chat.go` (`WebAgentChat`, `webAgentRunContext`, `acquireWebAgentSlot`) | No | Bounds total web-agent request time; must be between 1s and 120s. Also sizes the Redis concurrency-slot TTL (`Timeout + 30s`, min `1m`). Gateway HTTPRoute timeout is handled separately (see `/api/agent` HTTPRoute rule, 1800s). |
| `BI_WEB_AGENT_WORKSPACE_ALLOWLIST` | falls back to `BI_AGENT_WORKSPACE_ALLOWLIST` | N/A | No | `internal/config/config.go`, `internal/http/handlers/ai_agent_chat.go` (`WebAgentChat` → `workspaceAllowed`) | No | Comma-separated workspace IDs allowed to use web agent mode. If unset, reuses `BI_AGENT_WORKSPACE_ALLOWLIST`; empty means all workspaces. |
